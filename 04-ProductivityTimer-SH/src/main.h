...
/* forward declaration for protimer_t structure because it is declared below but
we have to use it in typedef alias for function pointer, if there is something else
which can cause problem, then we can use this forward declaration method */
struct _protimer_state_t;
typedef event_status_t (*protimer_state_t)( struct _protimer_state_t  *const , event_t const *const);

/* Main Application Structure */
typedef struct _protimer_state_t
{
  uint32_t          curr_time;
